...
class Apriori(object):
    ...
    def getLiftRatio(self, lhs, rhs):
        """ Get the lift ratio for a rule """
        support_lhs_rhs = self.getSupport(lhs.union(rhs))
        if support_lhs_rhs != 0:
            return self.getSupport(lhs.union(rhs)) / (self.getSupport(lhs) * self.getSupport(rhs))
        else:
    ...
    def create_bar_chart(self, data):
       # Sort data by value in descending order and get the top 5 items
        ...
        plt.bar(range(len(sorted_data)), sorted_data.values(), color='#efcc00')
        plt.xlabel('Judul Buku')
        plt.ylabel('Frekuensi')
        plt.title('Top 3 Buku Paling Banyak Dipinjam')
        plt.xticks(range(len(sorted_data)), sorted_data.keys(), rotation=0)
        plt.tight_layout()
